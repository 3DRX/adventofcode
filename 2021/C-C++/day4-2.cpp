...
int grids[GRIDNUM][5][5];
bool lit[GRIDNUM][5][5];
...
int main(void)
{
    ...
    while (true) {
        ...
        for (int i = 0; i < GRIDNUM; i++) { // lit the number called
            for (int j = 0; j < 5; j++) {
                for (int k = 0; k < 5; k++) {
    ...
    int sum = calSum(lastGrid);
    printf("winGrid: %d\n",winGrid);
    printf("sum: %d\n",sum);
    printf("command[ipt]: %d\n",command[ipt]);
    printf("%d\n", command[ipt] * sum);
    return 0;
}
