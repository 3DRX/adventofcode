...
#define L(i, j) (*L)[i][j]
  ...
  while (q.size() != 0) {
    ...
    pos p = q.front();
    q.pop();
    if (L(p.x, p.y).value == 'S' || L(p.x, p.y).value == 'a') {
      return L(p.x, p.y).distance;
    }
    if (p.x != 0 && (E(L(p.x, p.y).value) <= E(L(p.x - 1, p.y).value) + 1) &&
        !L(p.x - 1, p.y).visited) {
      q.push({p.x - 1, p.y});
    ...
    if (p.x != X - 1 &&
        (E(L(p.x, p.y).value) <= E(L(p.x + 1, p.y).value) + 1) &&
        !L(p.x + 1, p.y).visited) {
      q.push({p.x + 1, p.y});
      L(p.x + 1, p.y).visited = true;
      L(p.x + 1, p.y).distance =
    ...
    if (p.y != 0 && (E(L(p.x, p.y).value) <= E(L(p.x, p.y - 1).value) + 1) &&
        !L(p.x, p.y - 1).visited) {
      q.push({p.x, p.y - 1});
    ...
    if (p.y != Y - 1 &&
        (E(L(p.x, p.y).value) <= E(L(p.x, p.y + 1).value) + 1) &&
        !L(p.x, p.y + 1).visited) {
      q.push({p.x, p.y + 1});
      L(p.x, p.y + 1).visited = true;
      L(p.x, p.y + 1).distance =
